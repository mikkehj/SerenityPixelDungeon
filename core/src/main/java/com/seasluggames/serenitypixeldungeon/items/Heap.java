...
import com.seasluggames.serenitypixeldungeon.items.food.ChargrilledMeat;
import com.seasluggames.serenitypixeldungeon.items.food.FrozenCarpaccio;
import com.seasluggames.serenitypixeldungeon.items.food.MysteryMeat;
import com.seasluggames.serenitypixeldungeon.items.journal.DocumentPage;
		...
		if (type != Type.HEAP) {
			...
			for (Item item : items.toArray( new Item[0] )) {
				...
				if (item instanceof Potion) {
					...
					items.remove(item);
					((Honeypot.SerenityPot) item).destroyPot(pos);
