...
public class WndUseItem extends WndInfoItem {
	...
	public WndUseItem(final WndBag owner, final Item item ) {
		...
		if (Dungeon.hero.isAlive()) {
			ArrayList<PurpleButton> buttons = new ArrayList<>();
			for (final String action : item.actions( Dungeon.hero )) {
				
				PurpleButton btn = new PurpleButton( Messages.get(item, "ac_" + action), 8 ) {
					@Override
	...
	private static float layoutButtons(ArrayList<PurpleButton> buttons, float width, float y){
		ArrayList<PurpleButton> curRow = new ArrayList<>();
		float widthLeftThisRow = width;
		
		while( !buttons.isEmpty() ){
			PurpleButton btn = buttons.get(0);
			...
			if (buttons.isEmpty() || widthLeftThisRow <= 0 || curRow.size() >= 3){
				...
				while (widthLeftThisRow > 0){
					...
					ArrayList<PurpleButton> shortest = new ArrayList<>();
					PurpleButton secondShortest = null;
					...
					for (PurpleButton toGrow : shortest){
