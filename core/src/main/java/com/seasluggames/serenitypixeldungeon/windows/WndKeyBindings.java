...
import com.seasluggames.serenitypixeldungeon.ui.ScrollPane;
...
public class WndKeyBindings extends Window {
	...
	public WndKeyBindings() {
		...
		PurpleButton btnDefaults = new PurpleButton(Messages.get(this, "default"), 9){
			@Override
		...
		PurpleButton btnConfirm = new PurpleButton(Messages.get(this, "confirm"), 9){
			@Override
			protected void onClick() {
		...
		PurpleButton btnCancel = new PurpleButton(Messages.get(this, "cancel"), 9){
			@Override
			protected void onClick() {
		...
		private boolean onClick( float x, float y ){
			if (inside(x, y)){
				...
				if (x >= this.x + 3*width()/4 && key1 != 0) {
					SerenityPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, false, key2, key1));

				//assigning first key
				} else if (x >= this.x + width()/2){
					SerenityPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, true, key1, key2));
	...
	private class WndChangeBinding extends Window {
		...
		private PurpleButton btnConfirm;
		...
		public WndChangeBinding(GameAction action, BindingItem listItem, boolean firstKey, int curKeyCode, int otherBoundKey ){
			...
			PurpleButton btnUnbind = new PurpleButton(Messages.get(this, "unbind"), 9){
				@Override
				protected void onClick() {
			...
			btnConfirm = new PurpleButton(Messages.get(this, "confirm"), 9){
				@Override
				protected void onClick() {
					if (changedKeyCode != -1){
