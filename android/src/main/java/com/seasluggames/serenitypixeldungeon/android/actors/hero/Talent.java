...
import com.seasluggames.serenitypixeldungeon.android.actors.buffs.CounterBuff;
import com.seasluggames.serenitypixeldungeon.android.actors.buffs.EnhancedRings;
import com.seasluggames.serenitypixeldungeon.android.actors.buffs.FlavourBuff;
import com.seasluggames.serenitypixeldungeon.android.actors.buffs.Haste;
...
import com.seasluggames.serenitypixeldungeon.android.actors.buffs.WandEmpower;
...
import com.seasluggames.serenitypixeldungeon.android.effects.particles.LeafParticle;
...
public enum Talent {
    ...
    //Cleric T1
    HEALING_MEAL(128), STRONG_FAITH(129), DIVINE_TOUCH(130), MANTRA(131),
    //Cleric T2
    BLESSING_OF_DEXTERITY(132), BLESSING_OF_WIND(133), BLESSING_OF_LIFE(134), BLESSING_OF_LUCK(135), BLESSING_OF_WEALTH(136),
    //Cleric T3
    BLESSING_OF_REGENERATION(137, 3), BLESSING_OF_PROTECTION(138, 3),
    //Priest T3
    BLESSING_OF_POWER(139, 3), REJUVENATING_UPGRADE(140, 3), BLESSING_OF_RESTORATION(141, 3),
    //Monk T3
    BLESSING_OF_STRENGTH(142, 3), EMPOWER(143, 3), STEEL_SKIN(144, 3),

    //Gambler T1
    RISKY_MEAL(99), RISKY_UPGRADE(99), ALL_OR_NOTHING(99), LADY_LUCK(99),
    //Gambler T2
    SCALPER(99), AGGRESSOR(99), JACKPOT(99), BLUFFER(99), LONGSHOT(99),
    //Gambler T3
    FIXED_ODDS(99, 3), WISE_GUY(99, 3),
    //CardMaster T3
    ACE_OF_SPADE(99, 3), KING_OF_ALL_KINGS(99, 3), LADY_DESTINY(99, 3),
    //Dicer T3
    HOT_DICE(99, 3), SNAKE_EYES(99, 3), DOUBLE_PITCH(99, 3);
    ...
    public static void onFoodEaten(Hero hero, float foodVal, Item foodSource) {
        ...
        if (hero.hasTalent(HEALING_MEAL)) {
            ...
            if (hero.HP <= hero.HT / 3) {
                hero.HP = Math.min(hero.HP + 2 + 2 * hero.pointsInTalent(HEALING_MEAL), hero.HT);
                hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), 1 + hero.pointsInTalent(HEALING_MEAL));
                //3/4 HP healed, when hero is below 50% health
            } else if (hero.HP <= hero.HT / 2) {
                hero.HP = Math.min(hero.HP + 2 + hero.pointsInTalent(HEALING_MEAL), hero.HT);
                hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), hero.pointsInTalent(HEALING_MEAL));
            }
        }
        ...
        if (hero.hasTalent(RISKY_MEAL)) {
			int dice = Random.Int(0, 9);

			switch (hero.pointsInTalent(RISKY_MEAL)) {
				case 1:
					if (dice >= 6) {
						switch (dice) {
							case 6:
						...
						switch (dice) {
							case 0:
								Buff.affect(hero, Haste.class, 0.67f + hero.pointsInTalent(RISKY_MEAL));
								break;
							case 1:
								Buff.affect(hero, WellFed.class).reset();
								break;
							case 2:
								hero.HP += hero.pointsInTalent(Talent.RISKY_MEAL) * 5;
								hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), hero.pointsInTalent(RISKY_MEAL));
								break;
							case 3:
								hero.HP += hero.pointsInTalent(Talent.RISKY_MEAL) * 10;
								hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), hero.pointsInTalent(RISKY_MEAL));
								break;
							case 4:
								hero.HP += hero.pointsInTalent(Talent.RISKY_MEAL) * 15;
								hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), hero.pointsInTalent(RISKY_MEAL));
								break;
							case 5:
								hero.HP += hero.pointsInTalent(Talent.RISKY_MEAL) * 20;
								hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), hero.pointsInTalent(RISKY_MEAL));
								break;
						}
					}
					...
					if (dice >= 7) {
						...
						switch (dice) {
							...
							case 5:
								...
								break;
							case 6:
								hero.HP += hero.pointsInTalent(Talent.RISKY_MEAL) * 25;
								hero.sprite.emitter().burst(Speck.factory(Speck.HEALING), hero.pointsInTalent(RISKY_MEAL));
								break;
						}
					}
    ...
    public static void initClassTalents(HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents) {
        ...
        switch (cls) {
            ...
            case HUNTRESS:
                ...
                break;
            case CLERIC:
                Collections.addAll(tierTalents, HEALING_MEAL, STRONG_FAITH, DIVINE_TOUCH, MANTRA);
                break;
        }
